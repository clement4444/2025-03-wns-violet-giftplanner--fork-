...
type BuildShape<T extends readonly Entry[]> = {
    [P in T[number]as
    P extends VariableEnvListeType
    ? P
    : P extends readonly [infer K, any]
    ? K extends VariableEnvListeType ? K : never
    : never
    ]: P extends readonly [any, true] ? number : string;
};
...
export function getVariableEnvMulti<const T extends readonly Entry[]>(
    variableEnvName: T
): BuildShape<T> {
